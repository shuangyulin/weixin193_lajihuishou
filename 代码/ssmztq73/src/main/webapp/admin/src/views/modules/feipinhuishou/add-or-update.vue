<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="废品类型" prop="feipinleixing">
          <el-select :disabled="ro.feipinleixing" v-model="ruleForm.feipinleixing" placeholder="请选择废品类型">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="回收时间" prop="huishoushijian">
          <el-input v-model="ruleForm.huishoushijian" 
              placeholder="回收时间" clearable  :readonly="ro.huishoushijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="回收时间" prop="huishoushijian">
              <el-input v-model="ruleForm.huishoushijian" 
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="废品详细" prop="feipinxiangxi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.feipinxiangxi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.feipinxiangxi" label="废品详细" prop="feipinxiangxi">
                    <span v-html="ruleForm.feipinxiangxi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.huishoushijian = true;
          ...
          if(o=='feipinxiangxi'){
            this.ruleForm.feipinxiangxi = obj[o];
	    this.ro.feipinxiangxi = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.feipinxiangxi = this.ruleForm.feipinxiangxi.replace(reg,'../../../ssmztq73/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `feipinhuishou/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.feipinhuishouCrossAddOrUpdateFlag = false;
						 this.parent.search();
