...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','feipinleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','feipinleixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('feipinleixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussfeipinleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('feipinleixing', this.detail);
				this.$utils.msg('审核成功');
